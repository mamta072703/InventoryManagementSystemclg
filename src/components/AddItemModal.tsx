...
import { X, Plus, Package, Tag, AlertTriangle } from 'lucide-react';
...
interface AddItemModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (item: Omit<StationeryItem, 'id'>) => void;
}
...
export const AddItemModal: React.FC<AddItemModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (costPrice < 0 || sellingPrice < 0 || stock < 0) {
      setError('Prices and stock must be positive numbers');
      return;
    }

    const generatedSku =
      sku.trim() ||
      `${category.slice(0, 3).toUpperCase()}-${name
        .slice(0, 4)
        .toUpperCase()
        .replace(/\s+/g, '')}-${Math.floor(100 + Math.random() * 900)}`;
      ...
      <div className="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between p-4 sm:p-5 border-b border-slate-800">
          <div className="flex items-center space-x-2 text-white">
            ...
            <h2 className="font-bold text-base">Add New Stationery Product</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-5 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Faber-Castell 12 Pastel Highlighters"
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">
              Item Details / Supplier Note (Optional)
            </label>
            <textarea
              rows={2}
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="e.g. 100 gsm fine grain, supplied by Navneet distributors"
